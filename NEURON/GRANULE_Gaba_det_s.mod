...
NEURON {
	POINT_PROCESS GRANULE_Gaba_det_s
...
ASSIGNED {
	...
	tspike[101]	(ms)	: will be initialized by the pointprocess
	PRE[101]
	:PRE_2[500]
	numpulses
	t0
...
INITIAL {
	...
	FROM i=1 TO 101 { PRE[i-1]=0 tspike[i-1]=0 } :PRE_2[500]=0}
	tspike[0]=1e12	(ms)
	if(tau_1>=tau_rec){ 
...
NET_RECEIVE(weight,on, nspike,t0 (ms),x,y,z,u, tsyn (ms)) {
	INITIAL {
		x = 0
		y = 0
		z = 0
